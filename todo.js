...
class Circle {
  ...
  class Bomb {
    ...
    draw(canvas, ctx) {
      ctx.drawImage(this.pic,this.xx,755,canvas.width/13, canvas.height/10);
    }
    ...
    for(var c=0; c<brickColumnCount; c++) {
      for(var r=0; r<brickRowCount; r++) {
        ...
        if(b.status == 1) {
          ...
          var pic3 = new Image();
          pic3.src = "./images/ball4.png";
          ctx.drawImage(pic3, x-25,y+150,56,56);
          b.y -=2;
          if(b.y <= 125){
            b.status = 0;
          }
  ...
  class Bomb {
    ...
    draw(canvas, ctx) {
      ctx.drawImage(this.pic,this.xx,733,canvas.width/13, canvas.height/9);
    }
  ...
  function collisionDetection() {
    ...
    for(var c=0; c<brickColumnCount; c++) {
      for(var r=0; r<brickRowCount; r++) {
        var b = bricks[c][r];
        if(b.status == 1) {
          if(x > b.x - ballRadius && x < b.x + ballRadius && y+20 > b.y - ballRadius && y+ 20 < b.y + ballRadius) {
            b.status = 2;
            dy = -dy;
            score++;
          }
